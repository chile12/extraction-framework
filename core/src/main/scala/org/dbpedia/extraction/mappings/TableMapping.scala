    ...
    override def extract(tableNode : TableNode, subjectUri : String, pageContext : PageContext): Seq[Quad] =
    {
             ...
             templateNode <- createTemplateNode(rowNode, tableHeader) )
        {
            ...
            for (cls <- mapToClass.relatedClasses)
              graph += new Quad(context.language, DBpediaDatasets.OntologyTypes, instanceUri, context.ontology.properties("rdf:type"), cls.uri, rowNode.sourceUri, null, rowNode.line)
            ...
            for(corUri <- correspondingInstance)
            {
                ...
                graph += new Quad(context.language, DBpediaDatasets.OntologyProperties, corUri, correspondingProperty, instanceUri, rowNode.sourceUri, null, rowNode.line)
            }
