...
extends PageNodeExtractor
{
  ...
  override def extract(node : PageNode, subjectUri : String, pageContext : PageContext): Seq[Quad] =
  {
    ...
    quads += new Quad(language, DBpediaDatasets.SkosCategories, subjectUri, rdfTypeProperty, skosConceptClass.uri, node.sourceUri, null, node.line)
    quads += new Quad(language, DBpediaDatasets.SkosCategories, subjectUri, skosPrefLabelProperty, node.title.decoded, node.sourceUri, new Datatype("rdf:langString"), node.line)
    ...
    for(link <- collectCategoryLinks(node))
    {
      ...
      quads += quad(subjectUri, property, objectUri, link.sourceUri, link.line)
    }
