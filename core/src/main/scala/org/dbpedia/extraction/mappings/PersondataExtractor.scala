                    ...
                    case key if key == name =>
                    {
                        for(nameValue <- StringParser.parse(property))
                        {
                            ...
                            if (nameParts.size == 2)
                            {
                                ...
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, foafNameProperty, reversedName, property.sourceUri, new Datatype("rdf:langString"), property.line)
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, foafSurNameProperty, nameParts(0).trim, property.sourceUri, new Datatype("rdf:langString"), property.line)
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, foafGivenNameProperty, nameParts(1).trim, property.sourceUri, new Datatype("rdf:langString"), property.line)
                            }
                            else
                            {
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, foafNameProperty, nameValue.trim, property.sourceUri, new Datatype("rdf:langString"), property.line)
                            }
                            quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, rdfTypeProperty, foafPersonClass.uri, template.sourceUri, null, property.line)
                            nameFound = true
                        }
                    }
                    case _ =>
                ...
                for(property <- propertyList)
                {
                    property.key match
                    {
                        ...
                        case key if key == description =>
                        {
                            for(value <- StringParser.parse(property))
                            {
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, dcDescriptionProperty, value, property.sourceUri, new Datatype("rdf:langString"), property.line)
                            }
                        }
                        case key if key == birthDate =>
                        {
                            ...
                            {
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, birthDateProperty, date, property.sourceUri, datatype, property.line)
                            }
                        }
                        case key if key == deathDate =>
                        {
                            for ((date, datatype) <- getDate(property))
                            {
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, deathDateProperty, date, property.sourceUri, datatype, property.line)
                            }
                        }
                        case key if key == birthPlace =>
                        {
                            ...
                            {
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, birthPlaceProperty, objUri.toString, property.sourceUri, null, property.line)
                            }
                        }
                        case key if key == deathPlace =>
                        {
                            for(objUri <- objectParser.parsePropertyNode(property, split=true))
                            {
                                quads += new Quad(language, DBpediaDatasets.Persondata, subjectUri, deathPlaceProperty, objUri.toString, property.sourceUri, null, property.line)
                            }
                        }
                        case _ =>
                    }
